...
<html>
	...
	<body>
		...
		<div id="show-${domainClass.propertyName}" class="content_grails scaffold-show" role="main">
			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<li class="fieldcontain">
					...
					<%  } else if (p.oneToMany || p.manyToMany) { %>
						...
						<span class="property-value" aria-labelledby="${p.name}-label"><g:link controller="${p.referencedDomainClass?.propertyName}" action="exibir" id="\${${p.name[0]}.id}">\${${p.name[0]}?.encodeAsHTML()}</g:link></span>
						</g:each>
					<%  } else if (p.manyToOne || p.oneToOne) { %>
						<span class="property-value" aria-labelledby="${p.name}-label"><g:link controller="${p.referencedDomainClass?.propertyName}" action="exibir" id="\${${propertyName}?.${p.name}?.id}">\${${propertyName}?.${p.name}?.encodeAsHTML()}</g:link></span>
			...
			<g:form url="[resource:${propertyName}, action:'excluir']" method="DELETE">
				<fieldset class="buttons">
					<g:link class="edit" action="editar" resource="\${${propertyName}}"><g:message code="default.button.edit.label" default="Editar" /></g:link>
